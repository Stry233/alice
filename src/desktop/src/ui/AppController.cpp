...
void AppController::loadMappingFromFile(const QString &path) {
    if (autofocus_->loadMappingFromFile(path)) {
        log("AUTOFOCUS", "Mapping loaded from file");
...
QVariantList AppController::trackedFaces() const {
    // This would be called from QML to get face data for overlay rendering
    return {};
}
...
void AppController::onDepthChanged(float depthM, float confidence) {
    ...
    // Forward to autofocus (it may ignore if not active, but that's fine)
    autofocus_->processDepthData(depthM, confidence);
...
void AppController::onColorFrame(const QImage &frame) {
    ...
    // Run face detection if in face tracking mode
    if (autofocus_->focusMode() == FocusMode::FaceTracking && faceDetector_->isReady()) {
        auto detections = faceDetector_->detect(frame);
        auto tracked = subjectTracker_->update(detections, frame.width(), frame.height());

        if (!tracked.empty()) {
            auto &best = tracked.front();
            autofocus_->updateFaceTarget(depth(), depthConfidence());
            realsense_->setMeasurementPosition(
                static_cast<float>(best.focusPoint().x()),
                static_cast<float>(best.focusPoint().y()));
        }
    }
...
void AppController::onDepthFrame(const QImage &frame) {
    ...
    if (streamDepth_) {
        ...
        if (now - lastDepthSendMs_ >= minFrameIntervalMs_) {
            ...
            sendFrameToClient(kFrameTypeDepth, frame, streamQualityDepth_);
        }
    }
}
...
void AppController::onSyncMessage(const SyncMessage &message) {
    ...
    default:
        // Handle MEASURE_POSITION (not in enum — arrives as Heartbeat fallback)
        if (message.payload.contains("x") && message.payload.contains("y")) {
            float x = static_cast<float>(message.payload["x"].toDouble());
            float y = static_cast<float>(message.payload["y"].toDouble());
            fprintf(stderr, "[SyncServer] MEASURE_POSITION: x=%.3f y=%.3f\n", x, y);
            setMeasurementPosition(x, y);
            autofocus_->processTap(x, y);
        }
        break;
